...
Index writes (explicit `fuse index`, syntax-first cold start, watcher refreshes, and explicit semantic jobs) run through the repository job manager when `FUSE_DAEMON` is on (the default for `fuse mcp serve`). Matching callers join the same job; a semantic request can extend an active syntax job, while a force request conflicts and names the exact cancel command. Non-owner MCP clients call `fuse/openIndexed` over the pipe, then open `.fuse/fuse.db` read-only locally for queries. Daemon-less CLI commands (`fuse find`, `fuse index`) and `FUSE_DAEMON=0` serve use the same manager in process with the writer coordinator beneath it. Normal daemon-owned contention therefore returns a job snapshot rather than `index_busy:`.
...
The index can warm itself the moment a repo is served, before any tool call (R38): at `fuse host` (or in-process `fuse mcp serve`) start the daemon starts a syntax job for the served root, so the first read hits a warm or a bounded-building index rather than triggering a second cold job. It runs through the same repository job manager as CLI and MCP calls, so an eager start and a first read share one job. It is best-effort and default-on; opt out with `FUSE_EAGER_INDEX=0`, or warm explicitly with `fuse warm <path>`. The persistent store keeps a repo warm across a session (a reopened daemon is open-plus-reconcile, not a rebuild), and the idle-shutdown window holds the session daemon warm for quick successive sessions.
...
The daemon keeps the index fresh as the tree changes (R39). A debounced file-system watcher over the served root reconciles changed workspace files into the store through the single-writer coordinator, so a read finds the index already fresh and pays no per-read reconcile cost. It ignores `.git` and `.fuse`: inventory invokes Git commands that can update metadata, and Fuse's own derived files must not schedule another refresh. The reconcile is overlap-guarded (one at a time) and best-effort (a failure never tears down the daemon); a periodic safety reconcile catches events a watcher dropped (network drives), and on-read reconcile stays the backstop, so freshness never depends on the watcher being perfect. Default-on with the daemon; opt out with `FUSE_WATCH=0`.
...
### Preserving compiler facts during refresh

After `fuse index --semantic`, an eager warm, watcher tick, or on-read syntax request reuses the existing semantic/partial index when its manifest, extraction version, and current source hashes match. It leaves symbols, graph edges, target-framework availability, and load diagnosis intact. Validation and the subsequent write share the repository writer lock, so a queued syntax request rechecks the index after an earlier compiler writer finishes. A lightweight job may still appear in status; reuse does not run extraction.

When the inventory changes or the compiler index is incomplete, a syntax refresh discards the invalid derived index and rebuilds fresh syntax data. It reports `semantic-invalidated` in job warnings and records the reason in the load diagnosis. This also clears derived verification-session state. Run `fuse index --semantic` to restore compiler facts; background refreshes do not start compiler work automatically. `--force` continues to discard an unchanged index explicitly.
