...
{
    private readonly ConcurrentDictionary<string, RootGate> _gates = new(StringComparer.OrdinalIgnoreCase);

    /// <summary>
    ///     Incremented when this process acquires the cross-process writer lock (R19 single-writer audit tests).
    /// </summary>
    internal int ProcessWriteLockAcquireCount { get; private set; }
        ...
        int busyTimeoutMilliseconds = WorkspaceIndexConnectionFactory.DefaultBusyTimeoutMilliseconds)
    {
        ...
        var gate = _gates.GetOrAdd(normalizedRoot, _ => new RootGate());
